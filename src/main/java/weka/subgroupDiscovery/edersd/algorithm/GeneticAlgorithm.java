 ...
 *
 */
package weka.subgroupDiscovery.edersd.algorithm;
...
public class GeneticAlgorithm extends Algorithm {
	...
	@Override
	public EDERSD_Rule execute() throws Exception {
		...
		EDERSD_Rule m_RuleMaxMin = ((EDERSD_Rule) getInputParameter("ruleMaxMin"));
		...
		for (int ii = 0; ii < m_NumGenerations; ii++) {
			...
			for (int j = m_NumCopiedIndividuals; j < m_NumIndividuals; j++) {
				...
				EDERSD_Rule[] parent = new EDERSD_Rule[3];
				...
				parent[2] = m_CurrentPopulation.get(ind2);
					...
					if (ind1 < m_NumIndividuals && ind2 < m_NumIndividuals && 0 <= ind1 && 0 <= ind2) {
						rul_cross=(EDERSD_Rule) cros1.execute(parent);
					} else {
			...
			for (int j = 1; j < m_NumIndividuals; j++) {
				...
				EDERSD_Rule rul_mut = null;
				prob = m_RandomGenerator.nextFloat()*10;
				if (prob < m_ProbMutation) {	
					rul_mut=(EDERSD_Rule) mut1.execute(m_NextPopulation.get(j));
			...
			for (int i = 0; i < m_NumIndividuals; i++) {
				...
				if (m_NextPopulation.get(i) == null) {					
					EDERSD_Rule hr=new EDERSD_Rule(m_CurrentPopulation.get(i).getRuleClass());
					...
					if (j != m_ClassIndex) {
						...
						if (m_NextPopulation.get(i).getGene(j) instanceof Numeric) {
							...
							{							
								...
								throw new Exception("Error individual "
										+ String.valueOf(i) + ", attribute "
										+ String.valueOf(j));
							}							
