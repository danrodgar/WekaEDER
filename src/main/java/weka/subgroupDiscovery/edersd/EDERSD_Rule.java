...
public class EDERSD_Rule extends Rule {
	 ...
	 */
	public EDERSD_Rule(double ruleClass) {
		mClass = ruleClass;
	}
		...
		for (int h = 0; h < mNumAttributes; h++) {
			if (h != mClassIndex) {
				if (dataSetInstances.attribute(h).isNominal()) {
					...
					Enumeration<Object> list = dataSetInstances.attribute(h).enumerateValues();
	 ...
	 * @return boolean
	 */
	public boolean equalRule(EDERSD_Rule rule) {
	 ...
	 */
	@Override
	public EDERSD_Rule clone() {
		
		EDERSD_Rule rule = new EDERSD_Rule(mClass);
...
//				+ getCorrect() + ", Incorrect: " + this.getIncorrect();
	 ...
	 * @return true if the instance is contained, false if not
	 */
	public boolean coversRefined(Instance instance) {
		assert (instance != null);

		for (int i = 0; i < size(); i++) {
			if (i != mClassIndex) {
				if (getGene(i).containsRefined(instance, i) == false) {
					return false;
				}
			}
		}
	 ...
	 * @return
	 */
	public double nCondnotClass(ArrayList<Instance> setExample) {
			int n = 0;
			for (Instance instance : setExample) {
				if ((covers(instance))) {
					if (instance.classValue() != getRuleClass()) {
