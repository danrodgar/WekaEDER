...
import weka.classifiers.rules.hider.algorithm.*;
...
import weka.classifiers.rules.hider.operator.crossover.CrossoverFactory;
import weka.classifiers.rules.hider.operator.crossover.CrossoverOnePoint;
import weka.classifiers.rules.hider.operator.mutation.*;
import weka.classifiers.rules.hider.operator.selection.Selection;
import weka.classifiers.rules.hider.operator.selection.SelectionFactory;
import weka.classifiers.rules.preprocessing.InstancesManager;
import weka.core.*;
 ...
 * 
 */
public class HIDER extends AbstractClassifier implements
	...
	private int m_NumIndividuals = 6;
	private int m_NumGenerations = 2;
	private int m_NumCopiedIndividuals = 1;
	...
	private Fitness m_Fitness = FitnessFactory.getFunctionEvaluation("FunctionFitnessNormal"); // The fitness to use
	...
	private ArrayList<HIDER_Rule> m_Rules;
	private Instances m_DataSetInstances;
	private HIDER_Rule m_RuleMaxMin; // Rule with max and min value of instances.
	 ...
	 */
	public void HIDER() throws Exception {
		...
		// Initialize the set of data (This set will be reduce) with set
		// normalized.
		...
		m_Rules = new ArrayList<HIDER_Rule>(); // Set of rules

		m_RuleMaxMin = new HIDER_Rule(m_ClassOrder.get(0));
		...
		for (int class_index = 0; class_index < getDataSetInstances().classAttribute().numValues(); class_index++) 
		{
			...
			for (int kk = 0; kk < 10 && m_Data.size() > m_DataSetInstances.numInstances() - m_TargetInstances; kk++){
				...
				HIDER_Rule rule = m_Algorithm.execute();

				if (rule != null) 
				{
					rule = refineRule(rule, getDataSetInstances(), m_Data,mNumAttributes, m_RuleMaxMin);

					if (rule.getCorrect()>0)
					{
						m_Data = reduceData(rule, m_Data, m_TargetInstances);
						if (!equalsHIDER(rule, getRules())) {
							setRule(rule);
						}
					}
				}
		...
		// To add the rule max-min with the class that should stay with more
		// number of appearances
		if (!m_Data.isEmpty()) {
			HIDER_Rule rule = m_RuleMaxMin;
			rule.setClass(Double.parseDouble(countTargetInstance(getDataSetInstances(), m_Data)));
			rule = refineRule(rule, getDataSetInstances(), m_Data,mNumAttributes, m_RuleMaxMin);

			if (rule.getCorrect() > 0 || rule.getIncorrect() > 0) {
				m_Data = reduceData(rule, m_Data, m_TargetInstances);
				setRule(rule);
			}		
		}
...
//					 - range * 0.03);
	 ...
	 * @return boolean
	 */
	private boolean equalsHIDER(HIDER_Rule rule, ArrayList<HIDER_Rule> rules) {
		for (HIDER_Rule ruleAux : rules) {
	 ...
	 * @throws Exception
	 */
	private HIDER_Rule initInstanceGeneric(double targetClass,
			Instances mDataSetInstances, int mNumAttributes) throws Exception {
		HIDER_Rule auxRuleMaxMin = new HIDER_Rule(targetClass);
	 ...
	 *            Rule to be refined.
	 */
	private HIDER_Rule refineRule(HIDER_Rule individual,
			Instances m_DataSetInstances, ArrayList<Instance> m_Data,
			int mNumAttributes, HIDER_Rule ruleMaxMin) {
	 ...
	 * 
	 * @param individual
	 *            Rule to classify instances.
	 */
	private ArrayList<Instance> reduceData(HIDER_Rule individual,
			ArrayList<Instance> mData, int mTargetInstances) {
...
//			boolean is_contained = m_UseRefined ? individual.containsRefined(instance) : individual.contains(instance);
	...
	public String toString() {
		int i = 0;
		String result = "Random seed: " + String.valueOf(getRandomSeed())
		...
		if (getRules() != null) {
			for (i = 0; i < getRules().size(); i++) {
				result += "Rule "
						+ (i + 1)
						+ ":\n"
						+ getRules().get(i).toString(isUseRefined(),
								getDataSetInstances(), m_RuleMaxMin) + "\n\n";
			}
		} else {
	...
	/**
	 * Gets the current settings of the HIDER classifier.
	 * 
	...
	public TechnicalInformation getTechnicalInformation() {
		...
		result.setValue(Field.TITLE, "HIDER");
	 ...
	 * @return
	 */
	public ArrayList<HIDER_Rule> getRules() {
		return m_Rules;
	}
	 ...
	 * @param rules
	 */
	public void setRule(HIDER_Rule rules) {
		m_Rules.add(rules);
	}
	 ...
	 * @param rules
	 */
	public void setRules(ArrayList<HIDER_Rule> rules) {
		m_Rules = rules;
	}
