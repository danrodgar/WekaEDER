 ...
 *
 */
package weka.classifiers.rules.preprocessing;
...
public class InstancesManager {
   ...
   * @return
   */
  public ArrayList<HIDER_Rule> denormalize(ArrayList<HIDER_Rule> normalizedInstances){
  	  
	  ArrayList<HIDER_Rule> rules=new ArrayList<HIDER_Rule>();
	  
	  for (HIDER_Rule rule:normalizedInstances)
	  {
		  try {			  			
   ...
   * @throws Exception
   */
  private HIDER_Rule convertRule(HIDER_Rule instance) throws Exception {	  	 
	  	 	  	  	  
	    HIDER_Rule rule=instance;
	    	...
	    	for (int i=0;i<instance.getCond().length-1;i++)
	    	{
	    		if (m_nClassIndex!=i)
	    		{
	    			...
	    			{		    			    	
	    				...
	    				{
	    					valueLower=deNormalizeValue(((Continuous)gen).getLowerLimit(), i);
	    					((Continuous)rule.getGene(i)).setLowerLimit(valueLower);
	    				}

	    				if (((Continuous)gen).getUpperLimit()!=-9999.9)
	    				{
	    					valueUpper=deNormalizeValue(((Continuous)gen).getUpperLimit(), i);		    			    			    			    		
	    					((Continuous)rule.getGene(i)).setUpperLimit(valueUpper);
	    				}
	    			}
	    		}
	    	}
